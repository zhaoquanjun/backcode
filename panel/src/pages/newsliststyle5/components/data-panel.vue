<template>
  <div class="data-section setting-block__section">
    <div class="news-data-source panel-open__area">
      ...
      <div v-else class="source-data__list">
        <p v-show="categoryName == 'CategoryId'">
          ...
          分类的文章
          <template
            v-if="categoryIdList.length <= 4"
          >,当前至多选择<span>4</span>个文章</template>
        </p>
        <p v-show="categoryName == 'Id'">
          <template v-if="categoryIdList.length <= 4">
            <div>
              当前已选择
              <span>{{ categoryIdList.length }}</span>
              个文章
            </div>
          </template>
          <template v-else>
    ...
    <div class="news-detail-link panel-open__area">
      <h3 class="setting-block--subtitle">文章排序</h3>
...
import SetBannerPictureAttribute from './list-item-operate'
export default {
  components: {
    ArticlePopup,
