  ...
  <div class="data-section setting-block__section">
    <div class="news-data-source panel-open__area">
      ...
      <div v-else class="source-data__list">
        <p v-show="categoryName == 'ProductCategoryId'">
          当前已选择
          <span>{{
        ...
        <p v-show="categoryName == 'Id'">
          当前已选择
          <span>{{ categoryIdList.length }}</span> 个产品
        </p>
    ...
    <div class="news-detail-link panel-open__area">
      <h3 class="setting-block--subtitle">产品详情页选择</h3>
      <div class="link-select__area">
        <el-select
          v-model="productDetailPageId"
          placeholder="请选择"
          style="width: 100%;"
        >
          <el-option
...
<script>
import ProductPopup from '@comp/list/productPopup/index'
...
export default {
  ...
  computed: {
    ...
    productDetailPageId: {
      get: function() {
        return this.model.dataModel['productDetailPageId']
      },
      set: function(val) {
        const oldVal = this.model.dataModel['productDetailPageId']
        if (oldVal !== val) {
          const oldData = {}
          const data = {}
          oldData['productDetailPageId'] = oldVal
          data['productDetailPageId'] = val
          this.model.dataModel['productDetailPageId'] = val
          if (this.model.query.filters[0]['value'].length <= 0) {
            this.onDataChange('changeProductDetailPageId', oldData, data)
          } else {
  ...
  methods: {
    ...
    _handleSelectData() {
      const productList = this.allCategoryId.length > 0 ? [] : this.contentList
    ...
    addPriductList(val) {
      ...
      if (!flag) {
        ...
        if (!val.type) {
          ...
          data.filters[0]['columnName'] = 'ProductCategoryId'
