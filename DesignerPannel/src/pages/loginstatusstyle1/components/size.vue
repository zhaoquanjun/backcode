    ...
    _handleWidthInputChange(val) {
      ...
      setTimeout(() => {
        if (/^\d+$/.test(this.widthValue) || this.widthValue == '') {
          ...
          if (this.widthValue == '') {
            newVal = 'auto'
          } else if (parseInt(this.widthValue) > this.valueRange.width.maxValue) {
            newVal = this.valueRange.width.maxValue + 'px'
          } else if (parseInt(this.widthValue) < this.valueRange.width.minValue) {
            newVal = this.valueRange.width.minValue + 'px'
          } else {
            newVal = parseInt(this.widthValue) + 'px'
          }
          this.widthValue = parseInt(newVal)
          this._handleChangeValue(oldValue, newVal, 'width')
        } else {
          this.widthValue = this.height
          return false
        }
      }, 500)
    },
    ...
    _handleHeightInputChange(val) {
      ...
      setTimeout(() => {
        if (/^\d+$/.test(this.heightValue) || this.heightValue == '') {
          ...
          if (this.heightValue === '') {
            newVal = 'auto'
          } else if (parseInt(this.heightValue) > this.valueRange.height.maxValue) {
            newVal = this.valueRange.height.maxValue + 'px'
          } else if (parseInt(this.heightValue) < this.valueRange.height.minValue) {
            newVal = this.valueRange.height.minValue + 'px'
          } else {
