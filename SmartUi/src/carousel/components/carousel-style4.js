...
const NAME = 'carousel_style4'
const DATA_KEY = 'st.carouselstyle4'
import { getSysClass, getjQuery } from '../../util'
const $ = getjQuery()

const INNER_TEM_Style4 = `
...
templates[
  'style4'
] = `<div id="{{_Id}}" data-c-type="carousel" class="c-carousel slide {{_SysClass}}" data-c-style="{{_Style}}" data-c-effect="{{effect}}" data-c-autoplay="{{autoplay}}" data-config='{"carouselPaginationType":{{carouselPaginationType}}'>${INNER_TEM_Style4}</div>`

class CarouselStyle4 extends Base {
  ...
  refreshControl(data, callback) {
    ...
    let html = template(cacheKey, INNER_TEM_Style4, data)
  ...
  static carouselInterface(element, config, args) {
    ...
    if (typeof config === 'object') {
      _config = $.extend({}, CarouselStyle4.DEFAULTS, config)
    }

    const action = typeof config === 'string' ? config : _config.slide
    if (!data) {
      data = new CarouselStyle4($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      CarouselStyle4.carouselInterface(this, config, args)
    })
  }
}
const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = CarouselStyle4.jQueryInterface
$.fn[NAME].Constructor = CarouselStyle4
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return CarouselStyle4.jQueryInterface
}
$(window).on('load', function () {
  $(`[data-ride="${NAME}"]`)[NAME]()
})
export default CarouselStyle4
