...
const NAME = 'newslist_style2'
const DATA_KEY = 'st.newslist_style2'
import { getSysClass, getjQuery } from '../../util'
const $ = getjQuery()
const templates = {}
const INNER_LIST = `{{each listData item i}} 
                    <li class="list-item col-item{{percent}} col-md-item2" data-percent="{{percent}}">
                ...
                {{/each}}`
const INNER_TEM = `<ul class="c-newslist">${INNER_LIST}</ul>
                <div  class="pagination" data-ride="pagination" id="pgn_{{_Id}}" data-total="{{pageCount}}" data-relid="{{_Id}}" data-reltype="newslist">  
                </div>`
templates[
  'style2'
] = `<div id="{{_Id}}" data-c-type="newslist" data-date-style="{{dateStyle}}" data-c-style="{{_Style}}" data-query='{{_Query}}' class="c-newslist-container {{_SysClass}}" data-pgrelid="pgn_{{_Id}}" data-ride="newslist" data-conf='{"percent":"{{percent}}","target":"{{target}}","newsDetailPageId":"{{newsDetailPageId}}"}' >
                    ${INNER_TEM}
                </div>`

class NewsListStyle2 extends Base {
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      NewsListStyle2.newslistInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = NewsListStyle2.jQueryInterface
$.fn[NAME].Constructor = NewsListStyle2
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return NewsListStyle2.jQueryInterface
}
$(window).on('load', function () {
  $(`[data-ride="${NAME}"]`)[NAME]()
})

export default NewsListStyle2
