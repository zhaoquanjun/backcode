...
const NAME = 'newslist_style7'
const DATA_KEY = 'st.newslist_style7'
...
<li class="list-item col-item{{percent}} col-md-item{{mobilePercent}}" data-percent="{{percent}}">
    <a href="/news/{{if newsDetailPageId}}{{newsDetailPageId}}{{else}}empty{{/if}}/{{item.Id}}.html" target="{{target}}">
        ...
        <div class="list-content">
            ...
            <div class="view-more"><span class="view-detail">{{buttonName}}</span><span 
            class="date" data-date="{{item.PublishTime}}">{{item.PublishTime}}</span>
        </div>
        </div>
    </a>
</li>
...
class NewsListStyle7 extends Base {
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      NewsListStyle7.newslistInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = NewsListStyle7.jQueryInterface
$.fn[NAME].Constructor = NewsListStyle7
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return NewsListStyle7.jQueryInterface
}
$(window).on('load', function () {
  $(`[data-ride="${NAME}"]`)[NAME]()
})

export default NewsListStyle7
