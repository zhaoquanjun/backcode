...
const NAME = 'newslist_style5'
const DATA_KEY = 'st.newslist_style5'
import { getSysClass, getjQuery } from '../../util'
const $ = getjQuery()
const templates = {}
const INNER_LIST = ` 
<li class="c-newslist-item c-newslist__inner-left">
                        <a href="/news/{{if newsDetailPageId}}{{newsDetailPageId}}{{else}}empty{{/if}}/{{listData[0].Id}}.html" target="{{target}}">
                          <div class="c-newslist-item-picture">
                            <img src="{{listData[0].PictureUrl}}" alt="" class="object-fit--cover"   data-object-fit="cover">
                          </div>
                          <div class="c-newslist-item-info">
                            <div class="c-newslist-item-date date" data-date="{{listData[0].PublishTime}}">
                              <span class="date-day">{{listData[0].PublishDay}}</span>
                              <span class="date-year">{{listData[0].PublishYear}}</span>
                            </div>
                            <div class="c-newslist-item-content">
                              <h3 class="c-newslist-item-title ellipsis_line--multi">
                                {{listData[0].Title}}
                              </h3>
                              <p class="c-newslist-item-desc ellipsis_line--multi">
                                {{listData[0].Summary}}
                              </p>
                            </div>
                          </div>
                    ...
                    <li>
                      <ul class="c-newslist__inner-right">
                        ...
                        <li class="c-newslist-item">
                          <a href="/news/{{if newsDetailPageId}}{{newsDetailPageId}}{{else}}empty{{/if}}/{{item.Id}}.html" target="{{target}}">
                              <div class="c-newslist-item-info">
                                <div class="c-newslist-item-date date" data-date="{{item.PublishTime}}">
                                  <span class="date-day">{{item.PublishDay}}</span>
                                  <span class="date-year">{{item.PublishYear}}</span>
                                </div>
                                <div class="c-newslist-item-content">
                                  <h3 class="c-newslist-item-title ellipsis_line--multi">
                                    {{item.Title}}
                ...
                `
const INNER_TEM = `<div class="c-newslist-wrap"><ul class="c-newslist c-newslist__inner clearfix">${INNER_LIST}</ul></div>
...
class NewsListStyle5 extends Base {
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      NewsListStyle5.newslistInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = NewsListStyle5.jQueryInterface
$.fn[NAME].Constructor = NewsListStyle5
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return NewsListStyle5.jQueryInterface
}
$(window).on('load', function () {
  $(`[data-ride="${NAME}"]`)[NAME]()
})

export default NewsListStyle5
