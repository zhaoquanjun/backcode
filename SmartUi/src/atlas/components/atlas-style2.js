...
const NAME = 'atlas_style2'
const DATA_KEY = 'st.atlasstyle2'
import { getSysClass, getjQuery } from '../../util'
const $ = getjQuery()

const INNER_TEM_Style2 = ` {{each imgList item i}}    
                            <li class="list-item col-item{{percent}} col-md-item{{mobilePercent}}" data-slide-to="{{i}}">
                              <a class="linkArea" href="{{item.Href}}" target="_blank" data-type="{{item.Ctype}}" data-slide-to="{{i}}">       
                                <div class="list-space">
                                        ...
                                        <div class="list-content title ellipsis" {{if showTitle==false }} style="display:none" {{/if}}  >
                                            {{item.Name}}   
                                        </div>
                                    </div>  
                                    </a>
...
templates[
  'style2'
        ...
        <ul class="atlas-list">${INNER_TEM_Style2}</ul>
...
class AtlasStyle2 extends Base {
  ...
  refreshControl(data, callback) {
    ...
    let inner_template = INNER_TEM_Style2
  ...
  static atlasInterface(element, options, args) {
    ...
    if (!data) {
      data = new AtlasStyle2($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      AtlasStyle2.atlasInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = AtlasStyle2.jQueryInterface
$.fn[NAME].Constructor = AtlasStyle2
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return AtlasStyle2.jQueryInterface
}
$(window).on('load', function () {
  $(`[data-ride="${NAME}"]`)[NAME]()
})
export default AtlasStyle2
