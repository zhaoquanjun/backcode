...
const NAME = 'productlist_style4'
const DATA_KEY = 'st.productlist_style4'
...
templates[
  'style4'
] = `<div id="{{_Id}}" data-c-type="productlist" data-c-style="{{_Style}}" data-query='{{_Query}}'  data-pgrelId="pgn_{{_Id}}" data-ride="productlist"  data-conf='{"percent":"{{percent}}", "buttonName":"{{buttonName}}", "target":"{{target}}","productDetailPageId":"{{productDetailPageId}}"}'>
                      <div class="productlist-wrap">
                          ${INNER_TEM}
                      </div>
                  </div>`
export default class ProductListStyle4 extends Base {
  ...
  static productlistInterface(element, options, args) {
    ...
    if (!data) {
      data = new ProductListStyle4($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      ProductListStyle4.productlistInterface(this, config, args)
    })
  }
}
if ($) {
  const JQUERY_NO_CONFLICT = $.fn[NAME]
  $.fn[NAME] = ProductListStyle4.jQueryInterface
  $.fn[NAME].Constructor = ProductListStyle4
  $.fn[NAME].noConflict = () => {
    $.fn[NAME] = JQUERY_NO_CONFLICT
    return ProductListStyle4.jQueryInterface
  }
