...
const NAME = 'productlist_style3'
const DATA_KEY = 'st.productlist_style3'
...
const INNER_TEM = `<ul class="product-list product-list-style2">${INNER_LIST}</ul>
                   <div  
                      ...
                      data-relId="{{_Id}}" 
                      data-relType="productlist_style3">
                   </div>`

const templates = {}
templates['style3'] = `<div 
                        id="{{_Id}}" 
                        data-c-type="productlist" 
                        data-c-style="{{_Style}}" 
                        data-query='{{_Query}}'  
                        data-pgrelId="pgn_{{_Id}}" 
                        data-ride="productlist_style3"  
                        data-conf='{"percent":"{{percent}}", "buttonName":"{{buttonName}}", "target":"{{target}}","productDetailPageId":"{{productDetailPageId}}"}'>
                        <div class="productlist-wrap">
                          ${INNER_TEM}
                        </div>
                      </div>`
export default class ProductListStyle3 extends Base {
  ...
  static productlistInterface(element, options, args) {
    ...
    if (!data) {
      data = new ProductListStyle3($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      ProductListStyle3.productlistInterface(this, config, args)
    })
  }
}
if ($) {
  const JQUERY_NO_CONFLICT = $.fn[NAME]
  $.fn[NAME] = ProductListStyle3.jQueryInterface
  $.fn[NAME].Constructor = ProductListStyle3
  $.fn[NAME].noConflict = () => {
    $.fn[NAME] = JQUERY_NO_CONFLICT
    return ProductListStyle3.jQueryInterface
  }
