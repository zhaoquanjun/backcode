...
const DATA_KEY = 'st.productdetail'
const EVENT_KEY = `.${DATA_KEY}`
const NAME = 'productdetail'
const templates = {}
templates[
  'style1'
] = `<div id="{{_Id}}" data-c-type="{{_Name}}" class="c-productdetail {{_SysClass}} ql-bubble"><div class="ql-editor">{{Text}}</div></div>
...
class ProductDetail {
  ...
  static ProductDetailInterface(element, options, args) {
    const $this = $(element)
    let data = $this.data(DATA_KEY)
    if (!data) {
      data = new ProductDetail($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function() {
      ProductDetail.ProductDetailInterface(this, config, args)
    })
  }
}
const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = ProductDetail.jQueryInterface
$.fn[NAME].Constructor = ProductDetail
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return ProductDetail.jQueryInterface
}
