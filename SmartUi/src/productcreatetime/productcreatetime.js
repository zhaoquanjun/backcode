...
const NAME = 'productcreatetime'
const DATA_KEY = `st.${NAME}`
const templates = {}
templates[
  'style1'
] = `<div id="{{_Id}}" data-c-type="productcreatetime" contenteditable="false" class="c-productcreatetime {{_SysClass}}">{{Text}}</div>`

const $ = getjQuery()
class ProductCreateTime {
  constructor(id, style) {
    this._id = id
    this._type = 'productcreatetime'
    this._style = style
  ...
  changeDateStyle(data) {
    ...
    this._control.find('p').text(newTime)
  }
  ...
  // Static
  static productCreateTimeInterface(element, options, args) {
    const $this = $(element)
    let data = $this.data(DATA_KEY)
    if (!data) {
      data = new ProductCreateTime($this.attr('id'), 'style1')
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function() {
      ProductCreateTime.productCreateTimeInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = ProductCreateTime.jQueryInterface
$.fn[NAME].Constructor = ProductCreateTime
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return ProductCreateTime.jQueryInterface
}
$(window).on('load', function() {
  $(`[data-ride="${NAME}"]`)[NAME]()
})
export default ProductCreateTime
