...
templates[
  'style1'
] = `<div id="{{_Id}}" data-c-type="container" class="c-row dashed-line {{_SysClass}} c-section" data-conf='{"toggleSticky":{{toggleSticky}}}'>
...
class Container {
  ...
  toggleSticky(data) {
    ...
    if (data.toggleSticky) {
      // 设置吸顶效果 绑定滚动事件
      $(window)
        .off(Event.SCROLL_DATA_API)
        .on(
          Event.SCROLL_DATA_API,
      ...
      clearTimeout(this.timer)
      this.timer = null
      $(window).off(Event.SCROLL_DATA_API)
  ...
  static atlasInterface(element, options, args) {
    ...
    if (!data) {
      data = new Container($this.attr('id'), $this.attr('data-c-style'))
      data._init()
    }
  ...
  static jQueryInterface(config) {
    ...
    return this.each(function () {
      Container.atlasInterface(this, config, args)
    })
  }
}

const JQUERY_NO_CONFLICT = $.fn[NAME]
$.fn[NAME] = Container.jQueryInterface
$.fn[NAME].Constructor = Container
$.fn[NAME].noConflict = () => {
  $.fn[NAME] = JQUERY_NO_CONFLICT
  return Container.jQueryInterface
}
